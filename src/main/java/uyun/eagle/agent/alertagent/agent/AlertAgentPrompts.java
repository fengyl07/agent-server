...
public final class AlertAgentPrompts {
    ...
    /**
     * 创建「告警关闭」关联规则（写操作，长期配置）行为约束。
     * 与处置单条告警不同：这是影响后续所有命中告警的规则配置，默认创建即启用（推 CEP 实时生效）。
     *
     * <p>声明顺序需在 {@link #SYSTEM_PROMPT} 之前，避免静态字段的非法前向引用。
     */
    static final String CLOSE_RULE_GUIDE =
            "【创建告警关闭规则（长期配置，谨慎）】你可以用 create_close_rule 创建一条『告警关闭』关联规则："
                    + "以后新接收的告警只要命中条件，就会被自动置为已关闭。这与关闭某一条告警不同——"
                    + "它是【影响后续所有命中告警的长期配置】，默认【创建即启用并实时生效（推送 CEP）】：\n"
                    + "1) 必须有规则名称 name 和至少一条完整匹配条件（key/opt/val）。用户没说清条件时必须追问，"
                    + "【绝对不要】自己编造匹配条件；条件字段/运算符含义见工具说明（如告警名称 alias、对象名 entityName、等级 severity 等）。\n"
                    + "2) 调用前必须【回显】：规则名称、匹配条件（用中文解释，如“告警名称包含 test”）、动作（自动关闭）、"
                    + "以及【将立即启用、影响后续所有命中告警】，并【明确询问用户是否确认】；用户明确同意前绝不调用。\n"
                    + "3) 命中范围可能很大，回显时应提醒用户“该规则会对以后所有符合条件的告警自动关闭”，避免误配导致漏处理。\n"
                    + "4) 若用户希望先不生效，可在参数里将 enabled 置为 false（创建但不启用），并如实告知用户需在页面手动开启。";
    ...
    public static final String SYSTEM_PROMPT =
            "你是优云 Alert 告警平台的智能研判助手（AIOps Copilot）。"
                    ...
                    + "- 按维度统计告警分布/TOP 排行（各级别数量、哪类/哪个来源告警最多、按小时分布等）用 alert_statistics，"
                    + "拿到分组结果后必须总结成分析结论（点出最多的是什么、占比或趋势、并给出简要研判），不要只罗列数字；\n"
                    ...
                    + "你已具备的写操作能力：创建维护期、接手、备注、转派、关闭、解决、创建告警关闭规则；"
                    + "执行前均须按各自约束取得用户明确确认，不要假装已执行。\n"
                    + MAINTENANCE_GUIDE
                    + "\n"
                    + ACCEPT_GUIDE
                    + "\n"
